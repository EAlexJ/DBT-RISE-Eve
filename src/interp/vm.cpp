...
eve_vm::virt_addr_t eve_vm::execute_inst(finish_cond_e cond, virt_addr_t start,
  ...
  while (instr_count < icount_limit) {
    ...
    case op::MOV: {
      ...
      uint8_t *REG = reinterpret_cast<uint8_t *>(
          this->regs_base_ptr +
          arch::traits<
              eve_core>::reg_byte_offsets[arch::traits<eve_core>::REG0]);
      *NEXT_PC = *PC + 1;
    ...
    case op::ADD: {
      ...
      uint8_t *A = reinterpret_cast<uint8_t *>(
          this->regs_base_ptr +
          arch::traits<eve_core>::reg_byte_offsets[arch::traits<eve_core>::A]);
      uint8_t *B = reinterpret_cast<uint8_t *>(
          this->regs_base_ptr +
          arch::traits<eve_core>::reg_byte_offsets[arch::traits<eve_core>::B]);
      uint8_t *REG = reinterpret_cast<uint8_t *>(
          this->regs_base_ptr +
          arch::traits<
              eve_core>::reg_byte_offsets[arch::traits<eve_core>::REG0]);
      uint8_t *CY = reinterpret_cast<uint8_t *>(
          this->regs_base_ptr +
          arch::traits<eve_core>::reg_byte_offsets[arch::traits<eve_core>::CY]);
      uint8_t *SN = reinterpret_cast<uint8_t *>(
          this->regs_base_ptr +
          arch::traits<eve_core>::reg_byte_offsets[arch::traits<eve_core>::SN]);
      uint8_t *ZE = reinterpret_cast<uint8_t *>(
          this->regs_base_ptr +
          arch::traits<eve_core>::reg_byte_offsets[arch::traits<eve_core>::ZE]);
      uint8_t *OV = reinterpret_cast<uint8_t *>(
          this->regs_base_ptr +
          arch::traits<eve_core>::reg_byte_offsets[arch::traits<eve_core>::OV]);
      *NEXT_PC = *PC + 1;

      {
        ...
        *ZE = (res == 0);
        *OV = (int8_t)*A > 0 && (int8_t)*B > 0 && (int8_t)res < 0 ||
              (int8_t)*A < 0 && (int8_t)*B < 0 && (int8_t)res > 0;
        *A = (uint8_t)res;
      }
      break;
    }
    ...
    case op::GOTOXY: {
      ...
      uint8_t *X = reinterpret_cast<uint8_t *>(
          this->regs_base_ptr +
          arch::traits<eve_core>::reg_byte_offsets[arch::traits<eve_core>::X]);
      uint8_t *Y = reinterpret_cast<uint8_t *>(
          this->regs_base_ptr +
          arch::traits<eve_core>::reg_byte_offsets[arch::traits<eve_core>::Y]);
      *NEXT_PC = *PC + 1;
